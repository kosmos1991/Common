...
namespace Com.Business
{
    ...
    public class BaseBusiness<T> where T : class
    {
        ...
        /// <param name="where"></param>
        /// <returns></returns>
        public IBusinessResponse<IList<T>> GetListByWhere(Expression<Func<T, bool>> where)
        {
            var result = this.GetQueryable(where).ToList();

            return new BusinessResponse<IList<T>>(result);
        }
        ...
        /// <param name="where">查询条件.</param>
        /// <param name="selectExpression">查询表达式</param>
        /// <returns></returns>记录是否存在
        public IBusinessResponse<IList<TOut>> GetListByWhere<TOut>(Expression<Func<T, bool>> where, Expression<Func<T, TOut>> selectExpression)
        {
            var query_data = this.GetQueryable().Where(where).Select(selectExpression).ToList();

            return new BusinessResponse<IList<TOut>>(query_data);
        }
        ...
        /// <param name="entity">要新增的实体</param>
        /// <param name="IsSubmit">是否马上提交</param>
        public IBusinessResponse<bool> Add(T entity)
        {
            var resultEntity = context.Set<T>().Add(entity);
            ...
            if (rowNumber == 0)
            {
                return new BusinessResponse<bool>(false, "新增失败", "Not_Row", false);
            }
            return new BusinessResponse<bool>(true, "新增成功", "Success", true);
        }
        ...
        /// <returns></returns>
        public DataSet ToDataSet(string sql, SqlParameter[] prams)
        {
            return context.Database.ExecuteDataSet(sql, prams);
        }        
    }
}